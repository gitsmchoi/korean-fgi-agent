...
def generate_persona_response(persona, plan):
    ...
    persona_lens = ", ".join(
        [
            value
            for value in [
                safe_text(persona.get("job")),
                safe_text(persona.get("region")),
                safe_text(persona.get("family_persona")),
                safe_text(persona.get("professional_persona")),
            ]
            if value
        ]
    )
...
- 타깃 고객 설명보다 위의 실제 페르소나 정보와 관점 렌즈를 우선한다.
- 이 사람이 가진 직업, 지역, 가족/생활, 시간 제약, 소비 기준이 반응 문장에 구체적으로 드러나야 한다.
- 다른 응답자도 비슷하게 말할 법한 범용 표현을 피하고, 이 사람만의 판단 이유를 적는다.
- 직업이나 가족 정보에 없는 정체성은 붙이지 않는다. 예: 무직이면 워킹맘이라고 쓰지 않는다.
- positive_points, concerns, pain_points, needs, response_tags는 서로 같은 표현을 반복하지 않는다.
- 페르소나 정보는 반응의 근거로만 사용한다.
- 페르소나 설명을 그대로 요약하지 않는다.
- 분석 대상과 무관한 음식, 가족, 직장, 취미 이야기를 길게 확장하지 않는다.
- 실제 시장조사 결과처럼 단정하지 않는다.
- 각 배열 항목은 짧더라도 구체적인 명사와 상황을 포함한다.
- 자연스러운 한국어 문장으로 작성한다.
...
"""
    ...
    raw = ""
    try:
        raw = ask_llm(prompt, temperature=0.55, json_mode=True)
